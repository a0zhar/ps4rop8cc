...
js = os.popen(
    '8cc/python/rop-ps4-8cc /dev/stdout librop/*.c printf/*.c ps4/*.c '+ sources +
    ' | python3 bad_hoist/rop/compiler.py /dev/stdin bad_hoist/dumps/gadgets.txt'
    ).read()
js = ('''\
try
{
...
class RequestHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.send_header(
            'Content-Type', 'application/javascript' if self.path.endswith('.js') else 'text/html')
